...
class LinkTracker extends Model
{
    ...
    public function addShortenLink($username, $membershipId)
    {
        if (isset($_POST["website_link"]) && isset($_POST["csrf_token"])) {
            ...
            if ($membershipId != 1 && $shortenLinkSettings["paid_member_limit"] <= $this->totalUserShortenLink($username)) {
    ...
    public function trackingSystem()
    {
        if (isset($_GET["l"]) && !empty($_GET["l"])) {
            ...
            $this->insertData($this->shortenLinkClicksTable, ["shorten_code" => $_GET["l"], "username" => $shortenCodeDetails["username"], "visitor_country" => $user_country, "visitor_ip" => $visitor_ip, "visitor_timestamp" => time()]);
    ...
    public function updateSettings()
    {
        if (isset($_POST["free_member_limit"]) && isset($_POST["paid_member_limit"]) && isset($_POST["system_power"]) && isset($_POST["admin_csrf_token"])) {
            ...
            if (!is_numeric($_POST["system_power"]) || $_POST["system_power"] != 1 && $_POST["system_power"] != 2) {
                return ["success" => false, "message" => "Invalid status."];
            }
            if (!is_numeric($_POST["free_member_limit"]) || $_POST["free_member_limit"] < 1) {
                return ["success" => false, "message" => "Invalid limit for free membership."];
            }
            if (!is_numeric($_POST["paid_member_limit"]) || $_POST["paid_member_limit"] < 1) {
