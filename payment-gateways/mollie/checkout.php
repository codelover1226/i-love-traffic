...
if (isset($_GET["type"]) && isset($_GET["id"]) && isset($_GET["amount"])) {
    if (!empty($_GET["type"]) && !empty($_GET["id"]) && !empty($_GET["amount"])) {
        ...
        if (!empty($productType) && !empty($productName) && !empty($productId) && !empty($productPrice)) {
            ...
            $payment = $mollie->payments->create(["amount" => ["currency" => "USD", "value" => $productPrice], "description" => $productName, "redirectUrl" => $mainDomain . "dashboard.php?payment=success", "webhookUrl" => $mainDomain . "payment-gateways/mollie/ipn.php"]);
            $tmpTrxController = new TmpTransactionsController();
            $tmpTrxController->add(["username" => $username, "trx_id" => $payment->id, "payment_method" => "Mollie", "amount" => $productPrice, "product" => $productId, "product_type" => $productType, "website_link" => $websiteLink, "timestamp" => time()]);
