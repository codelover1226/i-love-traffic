...
if (isset($_GET["pause"])) {
    $flag = $bannerAd600400Controller->pauseUserAd($username, $_GET["pause"]);
} else if (isset($_GET["activate"])) {
    $flag = $bannerAd600400Controller->activateUserAd($username, $_GET["activate"]);
} else if (isset($_GET["delete"])) {
    $flag = $bannerAd600400Controller->deleteUserAd($username, $_GET["delete"]);
}
$membersController->generateUserCSRFToken();
$bannerAdsList = $bannerAd600400Controller->userBannerAdsList($username);
        ...
        <div class="row">
            ...
            <div class="col-xl-9">
                ...
                <div class="col-lg-12">
                        ...
                        <table class="table table-striped">
                            ...
                            <?php if (!empty($bannerAdsList)) : ?>
                                <?php foreach ($bannerAdsList as $adData) : ?>
                                    <tr>
                                        <td><a href="<?= $adData['image_link'] ?>" target="_blank"><img src="<?= $adData['image_link'] ?>" height="15" width="50" /></a></td>
                                        <td><?= strlen($adData["ad_link"]) > 15 ? substr($adData["ad_link"], 0, 15) . "..." : $adData["ad_link"] ?></td>
                                        <td><?= $adData["credits"] ?></td>
                                        <td><?= formatNumberDigits($adData["total_views"]) ?></td>
                                        <td><?= formatNumberDigits($adData["total_clicks"]) ?></td>
                                        <td>
                                            <?= $bannerAd600400Controller->adStatus()[$adData["status"] - 1] ?>
                                        </td>
                                        <td>
                                            <a href="web-banners-600-400.php?options=<?= $adData['id'] ?>">
                    ...
                    <?php $bannerAd600400Controller->userBannerAdsPagination($username) ?>
                </div>
