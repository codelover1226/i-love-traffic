...
if (isset($_GET["pause"])) {
    $flag = $loginAdController->pauseUserAd($username, $_GET["pause"]);
} else if (isset($_GET["activate"])) {
    $flag = $loginAdController->activateUserAd($username, $_GET["activate"]);
} else if (isset($_GET["delete"])) {
    $flag = $loginAdController->deleteUserAd($username, $_GET["delete"]);
}
$membersController->generateUserCSRFToken();
$loginAdsList = $loginAdController->userLoginAdsList($username);
                ...
                <div class="col-lg-12">
                    <div class="card border border-primary">
                        <table class="table table-striped">
                            ...
                            <?php foreach ($loginAdsList as $adData) : ?>
                            <tr>
                                ...
                                <td>
                                    <?= $loginAdController->adStatus()[$adData["status"] - 1] ?>
                                </td>
                                <td>
                                    <a href="web-logins.php?options=<?= $adData['id'] ?>">
                    ...
                    <?php $loginAdController->userLoginAdsPagination($username) ?>
                </div>
