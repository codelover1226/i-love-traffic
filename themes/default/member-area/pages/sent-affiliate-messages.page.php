...
$flag = $affiliateMessagingController->deleteSentMessage($userInfo["username"]);
$sentMessageList = $affiliateMessagingController->affiliateSentList($userInfo["username"]);
                ...
                <div class="col-lg-12">
                    <div class="card border border-primary">
                        <div class="card-header bg-transparent border-primary">
                            <h5 class="my-0 text-primary"><i class="mdi mdi-bullseye-arrow me-3"></i>Sent Affiliate Messages</h5>
                        </div>
                        <div class="card-body">
                            <div class="row">
                            ...
                            <table class="table table-striped">
                                <?php if (!empty($sentMessageList)) : ?>
                                    ...
                                    <?php foreach ($sentMessageList as $messageDetails) : ?>
                                        <tr>
                                            <td><a href="affiliate-message-details.php?id=<?= $messageDetails['id'] ?>"><?= base64_decode($messageDetails["message_subject"]) ?>
                                                    <?php if ($messageDetails["reading_status"] == 1) : ?>
                                                        <span class="badge bg-danger">Receiver Didn't Read Yet</span>
                                                    <?php endif; ?>
                                                </a>
                                            </td>
                                            <td><?= $messageDetails["receiver_username"] ?></td>
                                            <td><?= date("d, M Y h:i:s A", $messageDetails["sending_timestamp"]) ?></td>
                                            <td>
                                                <a href="sent-affiliate-messages.php?delete=<?= $messageDetails['id'] ?> &token=<?= $membersController->getUserCSRFToken() ?>" class="btn btn-danger waves-effect waves-light">Delete</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php else : ?>
                                    <div class="alert alert-danger" role="alert">
                                        No message yet ! Send some messages and build your network.
                                    </div>
                                <?php endif; ?>
                            </table>
                            <?= $affiliateMessagingController->affiliateSentPagination($userInfo["username"]) ?>
                        </div>
                    </div>
                </div>
