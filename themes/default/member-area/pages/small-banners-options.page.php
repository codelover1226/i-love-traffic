...
if (isset($_POST["image_link"])) {
    $flag = $smallBannerAdController->updateUserAd($username, $id);
} else if (isset($_POST["credits"])) {
    $flag = $smallBannerAdController->addUserAdCredits($username, $userInfo, $id);
} else if (isset($_POST["remove_credits"])) {
    $flag = $smallBannerAdController->removeBannerAdCredits($username, $id);
}
$bannerAdDetails = $smallBannerAdController->getBannerAdDetails($id);
                            ...
                            <div class="card border border-danger">
                                ...
                                <div class="card-body">
                                    <?php if ($bannerAdDetails["status"] != 1 && $bannerAdDetails["status"] != 3) : ?>
                                        <a href="small-banners.php?activate=<?= $id ?>&token=<?= $membersController->getUserCSRFToken() ?>"><button type="button" class="btn btn-success">Activate Ad</button></a>
                                    <?php endif; ?>
                                    <?php if ($bannerAdDetails["status"] != 2 && $bannerAdDetails["status"] != 3) : ?>
                                        <a href="small-banners.php?pause=<?= $id ?>&token=<?= $membersController->getUserCSRFToken() ?>"><button type="button" class="btn btn-info">Pause Ad</button></a>
                                    <?php endif; ?>
                                    <a href="small-banners.php?delete=<?= $id ?>&token=<?= $membersController->getUserCSRFToken() ?>"><button type="button" class="btn btn-danger">Delete Ad</button></a>
                                </div>
                            </div>
                        </div>
