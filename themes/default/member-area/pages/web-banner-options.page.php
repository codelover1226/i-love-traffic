...
if (isset($_POST["image_link"])) {
    $flag = $bannerAdController->updateUserAd($username, $id);
} else if (isset($_POST["credits"])) {
    $flag = $bannerAdController->addUserAdCredits($username, $userInfo, $id);
} else if (isset($_POST["remove_credits"])) {
    $flag = $bannerAdController->removeBannerAdCredits($username, $id);
}
$bannerAdDetails = $bannerAdController->getbannerAdDetails($id);
