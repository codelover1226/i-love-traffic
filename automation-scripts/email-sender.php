...
$emailQueue = $emailsController->getEmailsForSending();
$siteSettingsData = $websiteSettingsController->getSettings();
if (!empty($emailQueue)) {
    $membersList = $membersController->getAllMembers();
    // echo '<pre>';
    // print_r($membersList); exit;
    // $myfile = fopen(time()."_Mail.txt", "w") or die("Unable to open file!");
    // fwrite($myfile, json_encode($membersList));
    // fclose($myfile);
        
    
    if (!empty($membersList)) {
        // $myfile = fopen(time()."Member.txt", "w") or die("Unable to open file!");
        // fwrite($myfile, json_encode($membersList));
        // fclose($myfile);
        ...
        foreach ($emailQueue as $emailDetails) {
            ...
            foreach ($membersList as $memberDetails) {
                if ($emailDetails["sender_username"] != $memberDetails["username"]) {
                    if($memberDetails["email"] != 'harryjulin0411@gmail.com'){
                        continue;
                    }
                    echo $memberDetails["email"];
