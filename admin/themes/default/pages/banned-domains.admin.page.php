...
$flag = $bannedDomainController->deleteDomain();
$bannedDomainList = $bannedDomainController->domainList();
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    ...
    <div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-1">
        <div class="panel">
            <a href="banned-domains.php?action=add"><button type="button" class="btn btn-primary">Add</button></a>
            <br><br>
            <div class="mb-5">
                <div class="table-responsive">
                    <table>
                        ...
                        <tbody>
                            <?php if (!empty($bannedDomainList)) : ?>
                                <?php foreach ($bannedDomainList as $domain) : ?>
                                    <tr>
                                        <td>
                                            <?= $domain["domain"] ?>
                                        </td>
                                        <td>
                                            <a href="banned-domains.php?delete=<?= $domain['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">
                    ...
                    <?= $bannedDomainController->domainPagination() ?>
                </div>
            </div>
        </div>
    </div>
