...
$flag = $downlineBuilderController->deleteUserProgram_Admin();
$programList = $downlineBuilderController->getUserSpecificPrograms_Admin();
            ...
            <form action="" method="GET">
                <div class="form-group row">
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <button type="submit" class="btn btn-primary mt-6">Search</button>
                        </div>
                    </div>
                </div>
            </form>
                ...
                <div class="table-responsive">
                    <?php if (!empty($programList)) : ?>
                        <table>
                            ...
                            <tbody>
                                <?php foreach ($programList as $programData) : ?>
                                    <?php if ($downlineBuilderController->is_url_image($programData["affiliate_banner"])) :  ?>
                                        <tr>
                                            <td><a href="<?= $programData['affiliate_link'] ?>" target="_blank"><img src="<?= $programData['affiliate_banner'] ?>" style="height: 60px !important; width: 468px !important;"></a></td>
                                            <td class="text-center">
                                                <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                    ...
                                                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                        <li> <a class="dropdown-item" href="downline-builder-search.php?delete=<?= $programData['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Delete Program</a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </td>
                    ...
                    <?php else : ?>
                        <?php if (isset($_GET["username"]) && !empty($_GET["username"])) : ?>
                            <div class="alert alert-danger">Couldn't find any affiliate program in downline builder.</div>
                        <?php endif; ?>
                    <?php endif; ?>
                </div>
            </div>
