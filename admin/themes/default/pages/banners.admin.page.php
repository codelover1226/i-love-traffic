...
$flag = $membersController->deleteBanner();
$banners = $membersController->promotionalBannerList();
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    ...
    <div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-1">
        <div class="panel">
            <a href="banners.php?action=add"><button type="button" class="btn btn-primary">Add New</button></a><br>
            <div class="mb-5">
                        ...
                        <tbody>
                            <?php if (!empty($banners)) : ?>
                                <?php foreach ($banners as $banner) : ?>
                                    <tr>
                                        <td>
                                            <a href="<?= '../banners/' . $banner ?>" target="_blank"><img src="<?= '../banners/' . $banner ?>" /></a>
                                        </td>
                                        <td class="text-center">
                                            <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                ...
                                                <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                    <li> <a href="banners.php?banner=<?= trim($banner) ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Delete</a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </td>
