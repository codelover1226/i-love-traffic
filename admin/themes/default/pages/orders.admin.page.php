...
if (isset($_GET["unpaid"])) {
    $flag = $ordersController->markOrderAsUnpaid();
} else if (isset($_GET["paid"])) {
    $flag = $ordersController->markOrderAsPaid();
}
$ordersList = $ordersController->orderList();
        ...
        <div class="panel">
            <div class="mb-5">
                <div class="table-responsive">
                    <table>
                        ...
                        <tbody>
                            <?php if (!empty($ordersList)) : ?>
                                <?php foreach ($ordersList as $orderData) : ?>
                                    <tr>
                                        <td><?= $orderData["product_title"] ?></td>
                                        <td>$<?= $orderData["product_price"] ?></td>
                                        <td><?= $orderData["buyer_username"] ?></td>
                                        <td><?= $orderData["affiliate_username"] ?></td>
                                        <td>$<?= $orderData["affiliate_commission"] ?></td>
                                        ...
                                        <td class="text-center">
                                            <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                ...
                                                <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                    <?php if ($orderData["payment_status"] != 1) : ?>
                                                        <li><a href="orders.php?paid=<?= $orderData['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>"><button class="badge badge-info">Mark as Paid</button></a></li>
                                                    <?php else : ?>
                                                        <li><a href="orders.php?unpaid=<?= $orderData['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>"><button class="badge badge-danger">Mark as Unaid</button></a></li>
                                                    <?php endif; ?>
                                                </ul>
                                            </div>
                                        </td>
                    ...
                    <?= $ordersController->orderPagination() ?>
                </div>
            </div>
        </div>
    </div>
