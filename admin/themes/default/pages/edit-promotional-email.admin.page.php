...
$emailDetails = $promotionalsEmailsController->getEmailDetails($_GET["edit"]);
if (!empty($emailDetails)) {
    $flag = $promotionalsEmailsController->updateEmail($_GET["edit"]);
    $emailDetails = $promotionalsEmailsController->getEmailDetails($_GET["edit"]);
}
                ...
                <?php if (!empty($emailDetails)) : ?>
                    <form action="" method="POST">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="email_subject" value="<?= base64_decode($emailDetails['email_subject']) ?>" placeholder="Enter a subject for your email">
                            <input type="hidden" name="admin_csrf_token" value="<?= $adminController->getAdminCSRFToken() ?>">
                        </div>
                        <div class="form-group">
                            <label for="noticeContent">Email Body</label>
                            <textarea id="email_content" class="form-input" rows="30" name="email_body" value="<?= base64_decode($emailDetails['email_body']) ?>" placeholder="Enter your email content"><?= base64_decode($emailDetails['email_body']) ?></textarea>
                        </div>
