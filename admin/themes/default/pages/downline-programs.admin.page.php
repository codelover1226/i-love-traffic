...
if (isset($_GET["delete"]) && isset($_GET["token"])) {
    $flag = $downlineBuilderController->deleteAdminProgram();
}
$downlineProgramList = $downlineBuilderController->adminProgramList();
            ...
            <div class="mb-5">
                <div class="table-responsive">
                    <table>
                        ...
                        <tbody>
                            <?php if (!empty($downlineProgramList)) : ?>
                                <?php foreach ($downlineProgramList as $downlineProgramDetails) : ?>
                                    <?php if ($downlineBuilderController->is_url_image($downlineProgramDetails["affiliate_banner"])) : ?>
                                        <tr>
                                            <td><a href="<?= $downlineProgramDetails['affiliate_link'] ?>" target="_blank"><img src="<?= $downlineProgramDetails['affiliate_banner'] ?>" style="height: 60px !important; width: 468px !important;"></a></td>
                                            <td class="text-center">
                                                <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                    ...
                                                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                        <li> <a class="dropdown-item" href="downline-programs.php?delete=<?= $downlineProgramDetails['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Delete Program</a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </td>
