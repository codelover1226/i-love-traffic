...
$flag = $paymentSettingsController->updateStripe();
$paymentSettings = $paymentSettingsController->getSettings("Stripe");
        ...
        <div class="panel">
            <div class="mb-5">
                <form action="" method="POST">
                    <label for="noticeContent">Public Key</label>
                    <input type="text" class="form-input" name="public_key" placeholder="Stripe public key" value="<?= $paymentSettings['public_key'] ?>">
                    <input type="hidden" name="admin_csrf_token" value="<?= $adminController->getAdminCSRFToken() ?>">
                    <label for="noticeContent">Private Key</label>
                    <input type="text" class="form-input" name="private_key" placeholder="Stripe private key" value="<?= $paymentSettings['private_key'] ?>">
                    <label for="noticeContent">Webhook Key</label>
                    <input type="text" class="form-input" name="ipn_api_key" placeholder="Stripe webhook key" value="<?= $paymentSettings['ipn_api_key'] ?>">
                ...
                <br>
                IPN/Webhook Link: <?= $siteSettingsData["installation_url"] ?>payment-gateways/stripe/ipn.php
            </div>
        </div>
    </div>
</div>
