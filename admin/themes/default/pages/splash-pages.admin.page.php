...
$flag = $splashPageController->deleteSplashPage();
$splashPages = $splashPageController->splashPagesList();
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    ...
    <div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-1">
        <div class="panel">
            <a href="splash-pages.php?action=add"><button type="button" class="btn btn-primary">Add New</button></a><br>
            <div class="mb-5">
                <div class="table-responsive">
                    <table>
                        ...
                        <tbody>
                            <?php if (!empty($splashPages)) : ?>
                                <?php foreach ($splashPages as $splashPage) : ?>
                                    <tr>
                                        <td> <?= $splashPage["splash_page_name"] ?> </td>
                                        <td class="text-center">
                                            <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                ...
                                                <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                    <li><a href="splash-pages.php?action=edit&id=<?= $splashPage['id'] ?>">Edit</a> </li>
                                                    <li><a href="splash-pages.php?delete=<?= $splashPage['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Delete</a> </li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <?= $splashPageController->splashPagesPagination() ?>
                </div>
            </div>
        </div>
    </div>
