...
if (isset($_GET["approved"])) {
    $flag = $linkTrackerController->approveLink();
}
if (isset($_GET["pending"])) {
    $flag = $linkTrackerController->pendLink();
}
if (isset($_GET["ban"])) {
    $flag = $linkTrackerController->banLink();
}
                                ...
                                <?php if (!empty($linkList)) : ?>
                                    <?php foreach ($linkList as $linkDetails) : ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="link-trackers.php?details=<?= $linkDetails['shorten_code'] ?>&user=<?= $linkDetails['username'] ?>"><span class="badge bg-info">Details</span></a>
                                            </td>
                                            <td class="text-center">
                                                <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                    ...
                                                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                        <?php if ($linkDetails["status"] != 1) : ?>
                                                            <li><a class="dropdown-item" href="link-trackers.php?pending=<?= $linkDetails['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Pend Ad</a></li>
                                                        <?php endif; ?>
                                                        <?php if ($linkDetails["status"] != 2) : ?>
                                                            <li><a class="dropdown-item" href="link-trackers.php?approved=<?= $linkDetails['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Approve Ad</a></li>
                                                        <?php endif; ?>
                                                        <?php if ($linkDetails["status"] != 3) : ?>
                                                            <li><a class="dropdown-item" href="link-trackers.php?ban=<?= $linkDetails['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Ban Ad</a></li>
                                                        <?php endif; ?>
                                                        <li>
                                                            <a href="link-trackers.php?delete=<?= $linkDetails['shorten_code'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Delete</a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </td>
