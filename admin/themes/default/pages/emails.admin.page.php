...
if (isset($_GET["suspend"])) {
    $flag = $emailController->suspendMail();
} elseif (isset($_GET["unsuspend"])) {
    $flag = $emailController->unsuspendMail();
}
$emailList = $emailController->emailList();
        ...
        <div class="panel">
            <div class="mb-5">
                <div class="table-responsive">
                    <table>
                        ...
                        <tbody>
                            <?php if (!empty($emailList)) : ?>
                                <?php foreach ($emailList as $emailDetails) : ?>
                                    <tr>
                                        <td>
                                            <?= $emailDetails["sender_username"] ?>
                                        </td>
                                        <td>
                                            <?= base64_decode($emailDetails["email_subject"]) ?>
                                        </td>
                                        <td>
                                            <?= base64_decode($emailDetails["website_link"]) ?>
                                        </td>
                                        <td>
                                            <?= $emailDetails["total_sent"] ?>
                                        </td>
                                        <td>
                                            <?= $emailDetails["total_clicks"] ?>
                                        </td>
                                        <td>
                                            <?php if ($emailDetails["suspend_status"] == 1) : ?>
                                                <span class="badge bg-danger">Suspended<span>
                                                    <?php else : ?>
                                                        <span class="badge bg-success"><?= $emailController->emailStatus()[$emailDetails["email_status"]] ?><span>
                                                            <?php endif; ?>
                                        </td>
                                        <td class="text-center">
                                            <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                ...
                                                <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                    <?php if ($emailDetails["suspend_status"] != 1) : ?>
                                                        <li><a href="email-list.php?suspend=<?= $emailDetails['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Suspend</a></li>
                                                    <?php else : ?>
                                                        <li><a href="email-list.php?unsuspend=<?= $emailDetails['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Unsuspend</a></li>
                                                    <?php endif; ?>
                                                    <li><a href="email-list.php?more=<?= $emailDetails['id'] ?>">View</a></li>
                                                </ul>
                                            </div>
                                        </td>
                    ...
                    <?= $emailController->emailListPagination() ?>
                </div>
            </div>
        </div>
    </div>
