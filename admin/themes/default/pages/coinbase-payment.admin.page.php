...
$flag = $paymentSettingsController->updateCoinbase();
$paymentSettings = $paymentSettingsController->getSettings("Coinbase");
        ...
        <div class="panel">
            <div class="mb-5">
                <form action="" method="POST">
                    ...
                    <input type="text" class="form-input" name="ipn_api_key" placeholder="Coinbase Commerce API Key" value="<?= $paymentSettings['ipn_api_key'] ?>">
                    <input type="hidden" name="admin_csrf_token" value="<?= $adminController->getAdminCSRFToken() ?>">

                    <label for="noticeContent">Webhook Key</label>
                    <input type="text" class="form-input" name="private_key" placeholder="Coinbase Commerce Webook Key" value="<?= $paymentSettings['private_key'] ?>">
                ...
                <br>
                IPN/Webhook Link: <?= $siteSettingsData["installation_url"] ?>payment-gateways/coinbase/ipn.php
            </div>
        </div>
    </div>
</div>
