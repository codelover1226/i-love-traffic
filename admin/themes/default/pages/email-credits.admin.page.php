...
$flag = $emailCreditsController->deletePackage();
$packages = $emailCreditsController->packageList();
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    ...
    <div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-1">
        <div class="panel">
            <a href="email-credits.php?action=add"><button type="button" class="btn btn-primary">Add New</button></a><br>
            <div class="mb-5">
                <div class="table-responsive">
                            ...
                            <?php if (!empty($packages)) : ?>
                                <?php foreach ($packages as $package) : ?>
                                    <tr>
                                        ...
                                        <td>$<?= $package["price"] ?></td>
                                        <td><a href="../check-out.php?type=credits&id=<?= $package['id'] ?>">check-out.php?type=credits&id=<?= $package["id"] ?></a></td>
                                        <td>
                                        ...
                                        <td class="text-center">
                                            <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                ...
                                                <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                    <li><a href="email-credits.php?action=edit&id=<?= $package['id'] ?>">Edit</a> </li>
                                                    <li><a href="email-credits.php?delete=<?= $package['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>">Delete</a></li>
                                                </ul>
                                            </div>
                                        </td>
                    ...
                    <?= $emailCreditsController->packagePagination() ?>
                </div>
            </div>
        </div>
    </div>
