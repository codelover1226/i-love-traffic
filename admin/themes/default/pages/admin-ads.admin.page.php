...
if (isset($_GET["delete"]) && isset($_GET["token"])) {
    $flag = $adminAdsController->deleteAd();
}
$bannerAdList = $adminAdsController->adList();
        ...
        <div class="panel">
            <div class="mb-5">
                <div class="table-responsive">
                        ...
                        <tbody>
                            <?php if (!empty($bannerAdList)) : ?>
                                <?php foreach ($bannerAdList as $bannerAd) : ?>
                                    <tr>
                                        <td><a href="<?= $bannerAd['banner_link'] ?>" target="_blank" x-tooltip="<?= $bannerAd['banner_link'] ?>"><?= $adminAdsController->truncateString($bannerAd["banner_link"]) ?></a></td>
                                        <td><a href="<?= $bannerAd['website_link'] ?>" target="_blank" x-tooltip="<?= $bannerAd['website_link'] ?>"><?= $adminAdsController->truncateString($bannerAd["website_link"]) ?></a></td>
                                        ...
                                        <td class="text-center">
                                            <div x-data="dropdown" @click.outside="open = false" class="dropdown">
                                                ...
                                                <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:right-0 rtl:left-0">
                                                    <li><a href="admin-ads.php?action=edit&id=<?= $bannerAd['id'] ?>" @click="toggle">Edit</a></li>
                                                    <li><a href="admin-ads.php?delete=<?= $bannerAd['id'] ?>&token=<?= $adminController->getAdminCSRFToken() ?>" @click="toggle">Delete</a></li>
                    ...
                    <?= $adminAdsController->adListPagination() ?>
                </div>
            </div>
        </div>
    </div>
